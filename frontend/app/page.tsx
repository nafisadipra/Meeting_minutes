...
export default function Home() {
  const [view, setView] = useState<
    "meeting" | "profiling" | "documents" | "edit"
  >("meeting");
  ...
  const stopMeeting = async () => {
    ...
    try {
      ...
      if (
        data.new_profiles_discovered &&
        Object.keys(data.new_profiles_discovered).length > 0
      ) {
  ...
  const currentSessionName = currentSessionId
    ? (history.find((s) => s.id === currentSessionId)?.name ??
      "Meeting_Minutes")
    : "Meeting_Minutes";
    ...
    >
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm">
            ...
            >
              <h2 className="text-xl font-bold text-black mb-2">
                Unrecognized Voices Detected
              </h2>
              <p className="text-sm text-gray-700 mb-6 leading-relaxed">
                We identified new speakers during your session. Select the
                profiles to register to the local engine.
              </p>
              ...
              <div className="space-y-2 mb-8 max-h-[200px] overflow-y-auto pr-2">
                {Object.keys(discoveredProfiles).map((name) => (
                  <label
                    key={name}
                    className="flex items-center gap-4 p-3 bg-white border border-[#C4C4C4] rounded-md cursor-pointer hover:bg-gray-50 transition-colors"
                  >
                    <input
                      type="checkbox"
      ...
      <header className="flex justify-between items-start mb-6 shrink-0 print:hidden">
        <div className="flex items-center gap-4 ml-[10px]">
          ...
          <div className="h-[30px] flex items-center shrink-0">
            <img
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {isSettingsOpen && (
                ...
                >
                  {["Edit", "Pin", "Rename", "Save", "Delete Session"].map(
                    (option) => {
                      ...
                      const isCurrentlySaved =
                        currentSessionId &&
                        history.find((s) => s.id === currentSessionId)?.isSaved;

                      const displayOption =
                        option === "Pin" && isCurrentlyPinned
                          ? "Unpin"
                          : option === "Save" && isCurrentlySaved
                            ? "Remove from Vault"
                            : option;
          ...
          <nav className="w-[80px] flex flex-col items-center py-6 shrink-0 gap-6 bg-white z-20 relative border border-[#C4C4C4] rounded-md shadow-sm">
            <button
              ...
              className={`relative w-10 h-10 flex flex-col items-center justify-center rounded-md transition-all border border-[#C4C4C4] ${
                isHistoryOpen ? "bg-[#D9D9D9]" : "bg-white hover:bg-gray-50"
              }`}
                        ...
                        <button
                          ...
                          className={`w-full text-left p-3 mb-2 rounded-md flex flex-col gap-1 transition-colors outline-none border ${
                            currentSessionId === session.id
                              ? "bg-gray-50 border-[#C4C4C4] shadow-sm"
                              : "bg-white border-transparent hover:border-gray-200"
                          }`}
                        >
                          <div className="flex justify-between items-center w-full">
                            <span className="text-sm font-bold text-black truncate pr-2">
                              {session.name}
                            </span>
                            <div className="flex gap-1 shrink-0">
                              {session.isSaved && (
                                <svg
                                  className="w-3 h-3 text-[#5E5E5E]"
                                  fill="currentColor"
                                  viewBox="0 0 20 20"
                                >
              ...
              >
                ...
                <div className="w-1/3 border-r border-[#C4C4C4] flex flex-col overflow-hidden print:hidden bg-[white]">
                  <div className="p-6 pb-2 flex justify-between items-center z-10">
                    <h2 className="text-black font-bold uppercase text-xs tracking-widest">
                      Live Transcript
                    </h2>
                    {isRecording && (
                      <span className="text-xs font-bold text-black animate-pulse">
                        ● REC
                      </span>
                    )}
                  </div>
