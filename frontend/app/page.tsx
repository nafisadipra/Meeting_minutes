...
export default function Home() {
  ...
  // Stable refs so the polling interval doesn't need to be re-created on state changes.
  ...
  const poll = useCallback(async () => {
    try {
      ...
      // Backend unreachable — silently skip this tick.
    }
  }, []);
  ...
  const startMeeting = async () => {
    ...
    try {
      ...
      // Handled by polling — UI will reconcile on next tick.
    } finally {
      setIsStarting(false);
    }
  };
  ...
  const downloadMinutes = () => {
    ...
    // Defer revocation so the browser finishes the download before cleanup.
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-sm text-black/60 font-medium mb-6 leading-relaxed">
                New speakers were identified during your session. Select the ones you want to save permanently.
              </p>
        ...
        <div className="pt-6 border-t border-gray-700">
          <div className="p-3 bg-gray-700 rounded-xl">
            ...
            <p className="text-xs text-center text-white font-semibold flex items-center justify-center gap-1.5">
              <span className="w-1.5 h-1.5 bg-[#facc15] rounded-full animate-pulse" /> Local AI Online
            </p>
          </div>
        </div>
      </nav>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-center gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      disabled={isBusy}
                      className={`px-6 py-3 rounded-full text-sm font-bold transition-colors shadow-md disabled:opacity-60 ${
                        isRecording
                          ? "bg-black text-white shadow-black/20 animate-pulse"
                          : "bg-[#facc15] text-black shadow-yellow-400/30 hover:bg-[#eab308]"
                      }`}
                    >
                      <span className="flex items-center gap-2">
                        {isBusy ? (
                          <motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.8, ease: "linear" }} className="inline-block w-4 h-4 border-2 border-current border-t-transparent rounded-full" />
                        ) : isRecording ? (
                          <>Finish & Summarize <span className="text-base">⏹</span></>
                        ) : (
                          <>Start Meeting <span className="text-base">▶</span></>
                        )}
                      </span>
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 h-[500px]">
                  ...
                  <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                      {transcript.length === 0 && (
                        <p className="text-gray-500 text-sm italic font-medium">Listening for voice patterns...</p>
                      )}
                        ...
                        {transcript.map((line, i) => {
                          ...
                          return (
                            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-3 bg-gray-100 rounded-2xl border border-gray-200">
                              ...
                              <p className="text-sm text-black font-medium leading-relaxed">{rest.join("]:").trim()}</p>
                            </motion.div>
                          );
                        })}
                      </AnimatePresence>
                  ...
                  <div className="lg:col-span-3 bg-white p-6 rounded-3xl shadow-sm border border-gray-200 flex flex-col overflow-hidden relative">
                    ...
                    <div className="flex-1 overflow-y-auto bg-gray-100/50 p-6 rounded-2xl border border-gray-200 prose prose-sm prose-p:text-black prose-headings:text-black max-w-none">
                      {minutes ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                          ...
                          <p className="text-[#facc15] text-xs font-bold animate-pulse uppercase tracking-widest">Synthesizing minutes...</p>
                        </div>
                      ) : (
